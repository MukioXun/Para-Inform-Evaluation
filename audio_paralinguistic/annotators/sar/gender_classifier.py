...
class Bottle2neck(nn.Module):
    def __init__(self, inplanes: int, planes: int, kernel_size=None, dilation=None, scale: int = 8):
...
class GenderClassifier(BaseAnnotator):
    """性别分类标注器"""
    ...
    def load_model(self):
        """加载性别分类模型"""
        model_path = self.config.get('model_path', '/home/u2023112559/qix/Models/Models/gender-classifier')

        print(f"  [Gender] Loading model from: {model_path}")

        self.model = ECAPA_gender.from_pretrained(model_path)
    ...
    def annotate(self, audio_path: str) -> Dict[str, Any]:
        """执行性别预测"""
        gender, confidence = self.model.predict(audio_path, self.device)
