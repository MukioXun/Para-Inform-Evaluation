...
class QwenEvaluator:
    ...
    def _parse_response(self, response: str) -> Tuple[int, str]:
        ...
        # 提取分数: "The score is <1-5>"
        score_match = re.search(r'score\s+is\s+(\d)', response, re.IGNORECASE)
        if score_match:
            score = int(score_match.group(1))

        # 提取原因: "The reason is <...>"
        reason_match = re.search(r'reason\s+is\s+(.+?)(?:;|\.|$)', response, re.IGNORECASE)
        if reason_match:
...
def main():
    ...
    parser.add_argument(
        "--output", "-o",
        default="/home/u2023112559/qix/Project/Final_Project/Audio_Captior/qwen_eval_results",
